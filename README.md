...
- **Bug-triage batch + security fix (2026-08-01)** — 19-item hand-test bug list worked through in priority order on `fix/room-scan`. Most notably: **closed a real security exposure** — `GET /api/documents` had no auth at all, and RAG retrieval (`match_chunks`) never filtered on the `visibility` column migration 040 added for exactly this, so another user's private project document could surface verbatim in someone else's chat answer (migration `045`, code-complete — **needs a RAGAs run on the corpus machine before fully trusting it**, see STATE.md). Also: consolidated 5 duplicated voice-input implementations into one `useVoiceInput` hook; fixed the project switcher passing a whole object where the backend expects a plain id; shipped a real per-user Document Library + a staff petition-review UI (previously curl/Postman only); superadmin project visibility (inert rows + owner identity) with the two previously-inconsistent superadmin checks unified into one hook; and a kickoff-flow redesign adding free-form text/talk entry (AI-extracted, pre-fills the existing step wizard for review — never bypasses it). Full detail: `journals/session_20260801.md`.
- **⚠ README/STATE.md staleness note (2026-08-01):** this "Current Status" section and the TODO list below predate the document-upload plan (Type 1/2/3 petitions), the jurisdiction-accuracy overhaul, the contractor/bidding marketplace, and the room-scan/AR feature work — all shipped on this branch's history but never backfilled here. Out of scope for this session's health check to reconstruct; flagging so it isn't mistaken for "not yet done."
...
### In Progress
- [ ] **[Agent Architecture](docs/agent_architecture.md)** — 26-agent roster under one Manager, with prompt routing, a feedback loop, autonomy levels, and a token protocol. **Phases 0–4 are deployed to prod and CLOSED.** **Phase 5 (the course cut line) is CODE-COMPLETE on machine A (2026-07-26)** — all 8 components built + tested (575 pytest green): answer-level feedback loop, Performance Review #24, Evaluator #23, Citation Verifier #9, Query Deconstructor #5, Permit Strategy #11, Field Ontology core, and dashboard v2 (scorecard / corrections confirm-queue / autonomy). **Remaining tail:** deploy (apply migration `033` by hand + GHA), wire the three answer agents into the Manager's live query path, and run the batch eval loops + multi-sample RAGAs baseline on machine B. Phases 0–5 are the demo slice.
