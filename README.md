...
- **Agent architecture Phases 0–3 shipped to prod** — trace store, single-call-site runtime + registry, Manager + artifact store + Budget Governor, and the **Corpus Metadata Validator (agent #13) + backfill + superadmin dashboard v1** (`/admin/agents`: action queue, inline-editable metadata review, read-only Documents view). `/query/answer` runs behind the Manager. **Phase 4 core (Prompt Router + versioned fragment library + persona-aware `max_tokens`) is deployed to prod** (2026-07-25) — one question, three personas, three genuinely different answers. A query-UX pass (grounding-floor miss now a conversational 200 abstain instead of a 422 error, `persona_nudge` rendered, UI `top_k` 5→8) is **deployed to prod** (2026-07-25). Media Curator (#17) is next. See [docs/agent_architecture.md](docs/agent_architecture.md) and [STATE.md](STATE.md).
...
### In Progress
- [ ] **[Agent Architecture](docs/agent_architecture.md)** — 26-agent roster under one Manager, with prompt routing, a feedback loop, autonomy levels, and a token protocol. **Phases 0 (trace store), 1 (runtime + registry), 2 (Manager + artifact store + Budget Governor), and 3 (metadata validator + dashboard v1) are deployed to prod; Phase 4 core (Prompt Router + fragment library) is deployed to prod, along with the follow-on query-UX pass (conversational abstain + `persona_nudge`); the Media Curator agent is next.** Phases 0–5 are the demo slice.

### Planned
- [ ] [Agent Architecture Phase 4 — Media Curator (#17)](docs/plan_media_curator.md) — the deferred second pass of Phase 4: sourced how-to video links only (a curated `media_refs` table first, then `web_search` with `allowed_domains:["youtube.com"]`), Guardrail rejecting any URL from neither, wired into the `diy` path. **Slice B1 (curated table) is BUILT** on `feat/media-curator` (migration `030_media_refs`, deterministic diy-only curator, Guardrail zero-unsourced-URL gate, Manager wave-4 wiring, `media_refs` response + "📺 How-to videos" UI, seed script, tests; machine-A compile + offline smoke + frontend build green) — pending pytest + machine-B apply/seed/demo + deploy. **Slice B2 (`web_search`) deferred** (needs `run_agent` to learn `tools=`). Implementation plan: [docs/plan_media_curator.md](docs/plan_media_curator.md). **Slice C — video-transcript ingest** ([docs/plan_media_transcripts.md](docs/plan_media_transcripts.md)) makes DIY how-to queries *answerable* (they abstain today against a permit-code corpus): ingest the vetted videos' transcripts, embedded locally and stored as a **segregated, non-authoritative** retrieval class that never grounds or cites a compliance answer (so faithfulness ≥0.85 is untouched). The Router + fragment library + persona-aware `max_tokens` core is done (see Completed).
