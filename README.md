...
### In Progress
- [ ] **[Agent Architecture](docs/agent_architecture.md)** — 26-agent roster under one Manager, with prompt routing, a feedback loop, autonomy levels, and a token protocol. **Phases 0 (trace store), 1 (runtime + registry), 2 (Manager + artifact store + Budget Governor), 3 (metadata validator + dashboard v1), and 4 (Prompt Router + fragment library + query-UX pass + Media Curator #17) are deployed to prod and CLOSED.** **Phase 5 (answer agents — Query Deconstructor #5, Citation Verifier #9, Permit Strategy #11 — plus Evaluator #23, Performance Review #24, feedback UI, dashboard v2, and Field Ontology core) is next; start with the Evaluator + feedback loop and a multi-sample live RAGAs baseline.** Phases 0–5 are the demo slice.
