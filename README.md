...
### Planned
- [ ] [Agent Architecture Phase 4 — Media Curator (#17)](docs/media-curator-plan.md) — diy how-to links + segregated transcript answers; **never grounds compliance**. **On prod:** B1 links + C1 transcripts (`030`/`031`, RAGAs non-regression) + **C2** (diy compliance-abstain → grounded how-to answer + educational disclaimer) + **semantic links** (any ingested video surfaces with no per-video task_key) + **channel data** (This Old House, 18 videos, via `sync_how_to_to_prod`). **Deploy-pending (ops tooling):** channel crawl (`032`, RSS + `@handle` resolver), throttle + block-aware ingest, sync script — ff-merge `agents/phase-4`→`deployment/sites` to close Phase 4. **Deferred (not Phase 4):** B2 `web_search` (`run_agent tools=`), link liveness → Freshness #15, proxy for at-scale ingest, multi-sample RAGAs → Evaluator #23. Canonical plan: [docs/media-curator-plan.md](docs/media-curator-plan.md). Slice detail: [B1/B2](docs/plan_media_curator.md), [C1/C2](docs/plan_media_transcripts.md).
