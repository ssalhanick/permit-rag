...
### Planned
- [ ] [Agent Architecture Phase 4 — Media Curator (#17)](docs/media-curator-plan.md) — diy how-to links + segregated transcript answers; **never grounds compliance**. **B1 + C1 DEPLOYED** (prod: `030`/`031`, seeded links, transcripts, RAGAs non-regression). **C2 BUILT** (diy compliance-abstain → grounded how-to answer + educational disclaimer UI) — pytest + frontend green, **not deployed**. **B2 (`web_search`) deferred** (`run_agent` needs `tools=`). Half-2: deploy C2 → grow corpus → B2 → link liveness → citation polish → legal review. Canonical plan: [docs/media-curator-plan.md](docs/media-curator-plan.md). Slice detail: [B1/B2](docs/plan_media_curator.md), [C1/C2](docs/plan_media_transcripts.md).
