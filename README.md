...
### Planned
- [ ] [Agent Architecture Phase 4 — Media Curator (#17)](docs/plan_media_curator.md) — the deferred second pass of Phase 4: sourced how-to video links only (a curated `media_refs` table first, then `web_search` with `allowed_domains:["youtube.com"]`), Guardrail rejecting any URL from neither, wired into the `diy` path. Implementation plan: [docs/plan_media_curator.md](docs/plan_media_curator.md). The Router + fragment library + persona-aware `max_tokens` core is done (see Completed).
