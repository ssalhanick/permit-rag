...
### Planned
- [ ] [Agent Architecture Phase 3](docs/agent_architecture.md) — Corpus Metadata Validator + 27-doc backfill + superadmin dashboard v1 (action queue + metadata review). **Code landed on `agents/phase-3` (2026-07-24), pending machine-B verification + migration apply + deploy.** `ingestion/metadata_agent.py` (agent #13) runs deterministic enum/completeness checks first, then a single structured `run_agent` call for content-vs-metadata agreement + date extraction, every proposal carrying a source-chunk citation. Writes route through `ingestion/governance.py` only (`apply_metadata_correction`); failing docs draft only at ingest, never on the backfill (that would empty the live corpus). Migration is **028** (`028_metadata_validation.sql`), not 027 — 027 is `027_agent_action_item_dedupe`. Dashboard: `/admin/agents` (`api/routes/agents_admin.py` + `frontend/src/admin/`), superadmin-gated front and back. **Metadata source-of-truth**: corrected values land in the DB (the corpus's source of truth); `documents/metadata/*.json` sidecars stay gitignored local cache; `catalog.json` + `registry.json` remain the tracked source/governance artifacts.
