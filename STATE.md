...
| **`py -m pytest tests/ -q` "hang" (2026-08-04) — resolved, root cause was mundane** | What looked like an indefinite hang (killed after 20+ min; Scott's own run on the same machine got OOM-killed after 54 tests) resolved to three stacked, ordinary causes, not one exotic bug: (1) **Docker Desktop's daemon wasn't running at all** (not just the container down) — 6 tests genuinely need `db/client.py`'s real connection pool and failed with `psycopg_pool.PoolTimeout` after the pool's 30s wait, each hitting `127.0.0.1:5433` with nothing listening; `docker compose up -d` (per this file's own "Is the container up?" hint) fixed all 6 outright. (2) `rag/permit_classifier.py` defaults to `use_nli=True` and `rag/agents/registry.py` self-registers the real callable at import time — any test resolving `permit_classifier` without its own stub could trigger a real ~85MB HuggingFace NLI model load (`transformers`→`torch`/`sklearn`/`pandas`), which `test_permit_classifier.py` already mocked for itself but nothing else in the suite guarded against; this repo had **no `tests/conftest.py` at all**, so added one with an autouse fixture patching `_load_nli_classifier` to `None` suite-wide. (3) Most of the apparent multi-minute "hang" was **this session's own repeated concurrent bisection runs competing with Scott's simultaneously-running terminal invocation on the same physical machine** (`hostname` confirmed both were on `UTD95110`) — both processes independently importing the same heavy libraries at once. With Docker up and nothing else running concurrently, the full suite (832 tests) completes in **22 seconds** clean. Lesson: don't trust wall-clock-based bisection findings gathered under unknown concurrent load |
